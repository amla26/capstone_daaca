...
inp2=new Scanner(System.in);
			...
			//	System.out.println("Select source node");
		     ...
		     source=0; //Integer.valueOf(var3);
				...
				System.out.println("Select sink node");
				String var4=inp2.nextLine();
				destination=Integer.valueOf(var4);
				...
				for(int j=0;j<node_list[source].size();j++)
				 ...
				 for(int j=0;j<node_list[source].size();j++)
					{	
							p_temp1[j]=(Math.pow(n[source].pheromone[j].value,node.alpha)*Math.pow(n[source].r.eta[j].value,node.beta))/node.sum;
		 ...
		 {	/* Evaluate energy and update routing table and then select next node according to max p */
			 ...
			 for(int j=0;j<node_list[next_hop].size();j++)
				{	
						p_temp[j]=(Math.pow(n[next_hop].pheromone[j].value,node.alpha)*Math.pow(n[next_hop].r.eta[j].value,node.beta))/node.sum;
		 ...
		 {	int yes=1;
		 if(((destination==30)&&node_list[next_hop].get(k).id!=22&&node_list[next_hop].get(k).id!=20)||(destination==20&&node_list[next_hop].get(k).id!=30&&node_list[next_hop].get(k).id!=22)||((destination==22)&&node_list[next_hop].get(k).id!=30&&node_list[next_hop].get(k).id!=20))
	...
	for(int k=0;k<31;k++)
	{
		System.out.println("energy of node"+n[k].id+" -> "+n[k].E_curr);
