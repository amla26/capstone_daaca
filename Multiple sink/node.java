...
public class node {
	...
	{
		sum=sum+Math.pow(pheromone[i].value,alpha)*Math.pow(r.eta[i].value,beta);
	...
	{	
			...
			r.p[j].value=(Math.pow(pheromone[j].value,alpha)*Math.pow(r.eta[j].value,beta))/sum;
	...
	{	
		...
		for(int i=0;i<n.size();i++)
		{
			...
			r.eta[i].value=1/r.Edash_dist[i].
				value;
		...
		for(int i=0;i<n.size();i++)
		{
			r.p[i].value=(Math.pow(pheromone[i].value,alpha)*Math.pow(r.eta[i].value,beta))/sum;
		}
	...
	void adjust_pheromones(int n)
	{
		int k;
			...
			for(int j=0;j<n;j++)
			{
				r.eta[j].value = (1-rho_value)*r.eta[j].value;
			}
			// Deposit pheromones only on the path to neighbour with the highest energy
			
			r.eta[s].value=r.eta[s].value+r.E_dist[s].value;
	}
}
